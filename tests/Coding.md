# Coding Standards & Test Cases — UCCN / LEN System
**Code Quality Guidelines, Implementation & Test Specifications**
**Version:** 1.0 | **Date:** 2026-03-19 | **Sprint:** 3–4
...
- Docstring: ทุก class และ public method ต้องมี docstring
- Logging: ใช้ `logging` module แทน `print()`
- Type hints: บังคับใช้ใน function signatures

### 1.2 Naming Conventions

| Type | Convention | Example |
|------|-----------|---------|
| Class | PascalCase | `NetworkState`, `DAFTClassifier` |
| Function | snake_case | `find_best_route()`, `classify_node_pair()` |
| Constant | UPPER_SNAKE | `EFFICIENCY_THRESHOLD`, `HBAR_DAFT` |
| Parameter | snake_case | `node_id`, `threshold_rho` |
...
# Parameters: alpha (coupling), lambda (resolution)
# ==========================================
...
class DAFTClassifier:
    ...
    def cognitive_readiness(coherence: float) -> dict:
        ...
        C_PURE     = sqrt(2/3) ~0.817  -> ALLOW
        C_BOUNDARY = sqrt(1/3) ~0.577  -> floor
        """
        if coherence >= C_PURE:
    ...
    def asymmetry_decay(O4_0: float, t: float, tau: float = 1.0) -> dict:
        """
        O4(t) = O4(0) * e^(-t/tau)   -- DAFT Law 1 (classical)
        Returns t_star: time for asymmetry to reach PURE threshold (<0.01)
        """
...
EFFICIENCY_THRESHOLD  = 100.0   # ms
TRAFFIC_DENSITY_LIMIT = 5       # edges

class EvolutionEngine:
    """
    Monitors network and autonomously adapts topology:
    - Prunes inefficient routes (latency > EFFICIENCY_THRESHOLD)
    ...
    def _handle_high_density(self, network: NetworkState):
        ...
        if total_edges > TRAFFIC_DENSITY_LIMIT:
            logger.info(f"Evolution: high density ({total_edges} edges) — new path needed.")
...
class TestNodeLifecycle(unittest.TestCase):
    ...
    def test_is_available_when_degraded(self):
        """TC-NL-04: DEGRADED node is still reachable"""
        node = Node("N1")
        node.update_status(NodeStatus.DEGRADED)
...
class TestNetworkState(unittest.TestCase):
    ...
    def test_cyclic_graph_no_infinite_loop(self):
        """TC-NS-11: Cyclic topology terminates correctly"""
        self.net.add_edge("N1", "N2", 5)
...
class TestDAFTClassifier(unittest.TestCase):
    ...
    def test_asymmetry_decay(self):
        """TC-DAFT-08: O4 decays exponentially toward zero at t=10, tau=1"""
...
def run_demo():
    ...
    net.add_edge("Gamma", "Delta",  5)   # Alpha->Gamma->Delta = 10ms
...
|-------|-----------|---------|
| TestNodeLifecycle | TC-NL-01 ถึง TC-NL-05 | Node status lifecycle |
| TestNetworkState | TC-NS-01 ถึง TC-NS-14 | Topology + Dijkstra routing |
| TestEvolutionEngine | TC-EE-01 ถึง TC-EE-02 | Edge pruning |
| TestConsentManager | TC-CM-01 ถึง TC-CM-04 | Ethics consent |
| TestDAFTClassifier | TC-DAFT-01 ถึง TC-DAFT-09 | DAFT states + decay |
| **รวม** | **34 test cases** | **5 modules** |
...
```bash
# รัน demo + unit tests ทั้งหมด
python Coding.py

# รันเฉพาะ unit tests
python -m unittest Coding -v

# รันเฉพาะ suite เดียว
python -m unittest Coding.TestDAFTClassifier -v
```
